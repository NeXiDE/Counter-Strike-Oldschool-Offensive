...
SND_DZ_DAMAGE_1 <- "survival/creep_damage_01.wav";
SND_DZ_DAMAGE_2 <- "survival/creep_damage_02.wav";
SND_DZ_DAMAGE_3 <- "survival/creep_damage_03.wav";
SND_DZ_DAMAGE_4 <- "survival/creep_damage_04.wav";
SND_DZ_DAMAGE_5 <- "survival/creep_damage_05.wav";
...
function Precache()
{
	...
	self.PrecacheScriptSound( SND_DZ_DAMAGE_1 );
	self.PrecacheScriptSound( SND_DZ_DAMAGE_2 );
	self.PrecacheScriptSound( SND_DZ_DAMAGE_3 );
	self.PrecacheScriptSound( SND_DZ_DAMAGE_4 );
	self.PrecacheScriptSound( SND_DZ_DAMAGE_5 );
...
function HardModeToggle ( bool )
{
	...
	if (IsWarmupPeriod == false)
	{
		...
		HARD_MODE = false;
		debugPrint ("Starting the Second Mission");
		ScriptPrintMessageCenterAll( "Loading the second Mission" );
		...
		EntFire( "@missionbrief_01", "Disable", "", 0 );
...
{
    local GameMode = ScriptGetGameMode();
    local GameType = ScriptGetGameType();
    local MapName = GetMapName();
	
	if (GameMode != 1 || GameType != 4)
	SendToConsole("game_mode 1; game_type 4; changelevel " + MapName);
